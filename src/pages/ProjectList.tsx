...
const ProjectList: React.FC = () => {
  ...
  const [projectView, setProjectView] = useState(
    projects.slice(listNum * (currentPage - 1), listNum * currentPage),
  );
  ...
  const listSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    handlePageChange(1);
    setProjectView(
      filterView.slice(Number(e.target.value) * 0, Number(e.target.value) * 1),
    );
  };
  ...
  const filtering_status = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (status > -1) {
      const filter_result = projects.filter(
    ...
    <Container>
      <div>
        ...
        <table style={{ height: `${50 * (listNum + 1)}px` }}>
          ...
          <tbody>
            {projectView.map((project: any) => (
              <tr key={project.id}>
                <td>
                  <text onClick={() => nav(`/issuelist/` + project.id)}>
                    {project.title}
                  </text>
                </td>
        ...
        <div className="pagination">
          {currentPage > 1 ? (
            <span
              onClick={() => {
                ...
                setProjectView(
                  filterView.slice(
                    listNum * (currentPage - 2),
                    listNum * (currentPage - 1),
                  ),
                );
              }}
            >
          ...
          {currentPage}
          {currentPage < Math.ceil(filterView.length / listNum) ? (
            <span
              onClick={() => {
                handlePageChange(currentPage + 1);
                setProjectView(
